...
class CPW():
    ...
    def _pc0(self):
        return self._b/(2.*self._a*self._elliptic_integral(np.sqrt(1 - self._k0()**2))**2.)

    def _pc1(self):
        return 1. + np.log((8.*np.pi*self._a)/(self._a + self._b)) + (self._a * np.log(self._b/self._a))/(self._a + self._b)

    def _pc2(self):
        return self._pc1() - (2.*self._a*self._elliptic_integral(np.sqrt(1 - self._k0()**2))**2.)/self._b

    def _pc3(self):
        return (2.*self._b**2.*ellipe(np.sqrt(1.-self._k0()**2.)))/(self._a*(self._b + self._a)*self._elliptic_integral(np.sqrt(1-self._k0()**2)))
    ...
    def _F1(self):
        return self._F_up(self._t/2.) + self._elliptic_integral(self._k2())/self._elliptic_integral(np.sqrt(1-self._k2()**2)) - self._elliptic_integral(self._k1())/self._elliptic_integral(np.sqrt(1-self._k1()**2))

    def _F_up(self, t):

        if t<= self._s/2. :
            return self._elliptic_integral(self._k1())/self._elliptic_integral(np.sqrt(1-self._k1()**2)) + self._pc0()*( (t/self._s)*(self._pc1() - np.log((2.*t)/self._s)) +(t/self._s)*(t/self._s)*(1. - (3.*self._pc2())/2. + self._pc2()*np.log((2.*t)/self._s)))
        else :
            return self._elliptic_integral(self._k1())/self._elliptic_integral(np.sqrt(1-self._k1()**2)) + (self._pc0()*(self._pc2() + 2.))/8. + t/self._s

    def _F_low(self):
        return self._elliptic_integral(self._k1())/self._elliptic_integral(np.sqrt(1 - self._k1()**2))
    ...
    def _residual_optimal_gap_separation(self, gapWidth, targetImpedance, targetFrequency, verbose):
        ...
        if verbose:
            print 'Gap width:    '+str(abs(gapWidth[0]))+'    m', self.get_characteristic_impedance(targetFrequency) - targetImpedance
    ...
    def find_optimal_gap_separation(self, targetImpedance, targetFrequency, verbose=False):
        '''
            Calculate the optimal gap width separation in order to obtain a choosen impedance.
